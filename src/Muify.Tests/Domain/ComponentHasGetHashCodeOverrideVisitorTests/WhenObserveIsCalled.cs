namespace Muify.Domain.ComponentHasGetHashCodeOverrideVisitorTests;
...
public sealed class WhenObserveIsCalled
{
    ...
    public async Task GivenAComponentWhenHasGetHashCodeOverrideIsFalseThenEquatableToIdentifierDefinitionIsGenerated()
    {
        ...
        var visitor = new ComponentHasGetHashCodeOverrideVisitor();
        Component wheel = TestData.Single.Units.Value[0].Components[1].WithMetadata(metadata => metadata.HasGetHashCodeOverride(false));
        ...
        _ = await Assert.That(definition.Hint).IsEqualTo($"{wheel.Name}.GetHashCode");
    }
    ...
    public async Task GivenAComponentWhenHasGetHashCodeOverrideThenNothingIsGenerated()
    {
        // Arrange
        var visitor = new ComponentHasGetHashCodeOverrideVisitor();
        Component wheel = TestData.Single.Units.Value[0].Components[1].WithMetadata(metadata => metadata.HasGetHashCodeOverride(true));
        Model.Graph.Areas.Area.Units.Unit.Components.Component component = new(TestData.Single.Components, 0, TestData.Single.Model, wheel);
