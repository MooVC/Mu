namespace Muify.Domain
{
    ...
    internal sealed class ComponentIdentifierComparabilityHasLessThanOrEqualOperatorVisitor
          ...
          IModelVisitor<Model.Graph.Areas.Area.Units.Unit.Components.Component, File>
    {
        ...
        private static IEnumerable<File> Generate(Component component, Qualifier @namespace)
        {
            if (component.Metadata.Identifier.Comparability.IsComparable == Presence.NotApplicable
             || component.Metadata.Identifier.Comparability.HasLessThanOrEqualOperator
            ...
            var content = Builder
                ...
                .For<Class>(@class => @class
                    ...
                    .WithOperators(operators => operators
                        .WithComparisons(lessThanOrEqual => lessThanOrEqual
                            .To(component.Identifier.Type)
                            .WithBody("return left is not null && left.CompareTo(right) <= 0;")
                            .WithOperator(Comparison.Types.LessThanOrEqual)))
                    .WithScope(Scopes.Unspecified))
                .From(@namespace)
                .ToSnippet(Configuration.Options);

            yield return new File(content, $"{component.Name}.Identifier.Comparison.LessThanOrEqual");
        }
    }
}