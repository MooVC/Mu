namespace Muify.Domain
{
    ...
    internal sealed class ComponentIdentifierComparabilityIsComparableVisitor
          ...
          IModelVisitor<Model.Graph.Areas.Area.Units.Unit.Components.Component, File>
    {
        ...
        {
            if (component.Metadata.Identifier.Comparability.IsComparable != Presence.Missing || component.Identifier.IsUndefined)
            {
                yield break;
            }

            var identifier = component.Identifier.Type.ToSnippet(Configuration.Options.Types);

            var content = Builder
                .New<Definition>()
                .For<Class>(@class => @class
                    .DerivesFrom((Name: $"IComparable<{identifier}>", Qualifier: "System"))
